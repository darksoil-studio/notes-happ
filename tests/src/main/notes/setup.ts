...
import { NotesClient } from "../../../../ui/src/main/notes/notes-client.js";
import { NotesStore } from "../../../../ui/src/main/notes/notes-store.js";
...
async function addPlayer(scenario: Scenario) {
  const player = await scenario.addPlayerWithApp({ type: "path", value: appPath });
  ...
  const store = new NotesStore(
    new NotesClient(player.appWs as any, "main"),
  );
  return {
    store,
    player,
