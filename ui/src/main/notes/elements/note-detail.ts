...
export class NoteDetail extends SignalWatcher(LitElement) {
	...
	async updateNote() {
		...
		try {
			const promise = this.notesStore.client.updateNote(
				encodeHashToBase64(this.noteHash),
				title,
				content,
			);
				...
				<div class="column" style="gap: 16px; flex: 1; width: 558px">
					<div class="row" style="align-items: center">
						<span
							...
							contenteditable
							@input=${() => this.debouncedUpdateNote()}
						></span>
					</div>
	...
	async firstUpdated() {
		const note = await toPromise(
			this.notesStore.currentNote.get(this.noteHash),
		);
		setTimeout(() => {
			const titleEl = this.shadowRoot!.getElementById(
				'title',
			...
			effect(() => {
				const latestVersion = this.notesStore.currentNote
					.get(this.noteHash)
					.get();
	...
	render() {
		const note = this.notesStore.currentNote.get(this.noteHash).get();
