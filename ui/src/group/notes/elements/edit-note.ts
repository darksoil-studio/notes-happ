  ...
  renderEditForm(currentRecord: EntryRecord<Note>) {
    return html`
      <sl-card>
        ...
        <sl-textarea name="content" .label=${
      msg("Content")
    }  required .defaultValue=${currentRecord.entry.content}></sl-textarea>
        <upload-files name="image_hash" one-file accepted-files="image/jpeg,image/png,image/gif" required .defaultValue=${currentRecord.entry.image_hash}></upload-files>
