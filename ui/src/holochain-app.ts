...
export class HolochainApp extends SignalWatcher(LitElement) {
  ...
  router = new Router(this, [
        ...
        html`<overlay-page
				...
				>
					<create-group-profile
						@group-created=${(e: CustomEvent) => this.router.goto(`/home/group/${e.detail.roleName}`)}
					>
					</create-group-profile>
				</overlay-page>`,
    },
    ...
    // },
    {
      ...
      render: params =>
        html`<overlay-page
					...
					icon="close"
					@close-requested=${() => this.router.goto(`/group-profile/${params.groupProfileHash!}`)}
				>
					<edit-group-profile
						.groupProfileHash=${decodeHashFromBase64(params.groupProfileHash!)}
						@group-profile-updated=${() => this.router.goto(`/group-profile/${params.groupProfileHash!}`)}
					></edit-group-profile>
				</overlay-page>`,
    },
    {
      ...
      >
        <create-note
          @note-created=${(e: CustomEvent) => this.router.goto(`/note/${encodeHashToBase64(e.detail.noteHash)}`)}
        >
        </create-note>
      </overlay-page>`,
    },
    {
      path: "/note/:noteHash",
      render: (params) => {
        const store = this.shadowRoot?.querySelector<NotesContext>("notes-context")?.store;
        ...
        return html`<overlay-page
          ...
          >
            <note-detail
              ...
              @edit-clicked=${() => this.router.goto(`/note/${params.noteHash}/edit`)}
            >
            </note-detail>
          </overlay-page>`;
      },
    },
    {
      ...
      render: (params) =>
        ...
        .title=${msg("Edit Note")}
        icon="close"
        @close-requested=${() => this.router.goto(`/note/${params.noteHash!}`)}
      >
        <edit-note
          .noteHash=${decodeHashFromBase64(params.noteHash!)}
          @note-updated=${() => this.router.goto(`/note/${params.noteHash!}`)}
        ></edit-note>
      </overlay-page>`,
    },
  ]);
